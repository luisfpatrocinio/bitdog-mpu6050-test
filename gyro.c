...
static uint64_t last_update_time_us = 0; // Global variable to store the last update time in microseconds
...
CubeFace_e getCubeFace(float r, float p)
{
    ...
    float threshold_flat = 30.0f; // Ângulo para considerar "plano" (face Z para cima/baixo)
    float threshold_side = 70.0f; // Ângulo para considerar "de lado" (outras faces)

    if (pitch_abs < threshold_flat && roll_abs < threshold_flat)
    {
        return FACE_Z_POS; // Face Z+ para cima
    }
    else if (pitch_abs < threshold_flat && roll_abs > (180.0f - threshold_flat))
    {
...
void updateOrientation(MPU6050_data_t *data)
{
    ...
    float pitch_accel = atan2(-ax_g, sqrt(ay_g * ay_g + az_g * az_g)) * (180.0f / M_PI);

    // Atualizar roll e pitch com o filtro
    data->roll = ALPHA * (data->roll + gx_dps * dt) + (1.0f - ALPHA) * roll_accel;
    data->pitch = ALPHA * (data->pitch + gy_dps * dt) + (1.0f - ALPHA) * pitch_accel;
